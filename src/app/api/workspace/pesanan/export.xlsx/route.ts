...
import { toWorkspacePesananExportRows, toWorkspacePesananXlsxBuffer } from "@/server/workspace/pesanan-export";
import { withTiming } from "@/server/monitoring/with-timing";

export const runtime = "nodejs";

export const GET = withTiming("/api/workspace/pesanan/export.xlsx", async (request) => {
  try {
    ...
    const buffer = toWorkspacePesananXlsxBuffer(toWorkspacePesananExportRows(rows));
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="pesanan-${new Date().toISOString().slice(0, 10)}.xlsx"`,
      },
    });
  } catch (error) {
