...
export type TrendPoint = { period: string; totalSales: string; cos: string; com: string; pendapatanBersih: string };
...
export async function getPesananTrend(filter: OverviewDateFilter): Promise<TrendPoint[]> {
  ...
  const [salesRows, comRows] = await Promise.all([
    getDb().execute<{ period: string; total_sales: string; cos: string }>(sql`
      ...
      WHERE ${activeGenerationCondition("o")} AND ${notDeletedCondition("o")} AND o.status = 'CONFIRMED' AND o.source_order_id IS NOT NULL AND o.order_date BETWEEN ${from}::date AND ${to}::date
  ...
  return [...periods].sort().map((period) => {
    ...
    return { period, totalSales: totalSales.toString(), cos: cos.toString(), com: com.toString(), pendapatanBersih: calculatePendapatanBersih(totalSales, cos, com).toString() };
  });
}
...
export async function getPaymentComposition(filter: OverviewDateFilter): Promise<PaymentComposition[]> {
  ...
  const result = await getDb().execute<{ payment_method: "COD" | "TRANSFER"; closing: string; value: string }>(sql`
    ...
    FROM workspace_orders o WHERE ${activeGenerationCondition("o")} AND ${notDeletedCondition("o")} AND o.status = 'CONFIRMED' AND o.source_order_id IS NOT NULL AND o.order_date BETWEEN ${from}::date AND ${to}::date
...
export async function getOverviewKpi(filter: OverviewDateFilter): Promise<OverviewKpi> {
  ...
  return {
    ...
    cos: cos.toString(),
    com: com.toString(),
    pendapatanBersih: calculatePendapatanBersih(totalSales, cos, com).toString(),
  };
}
